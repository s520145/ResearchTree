...
public static class TooltipHandler_Modified
{
    ...
    {
        if (Event.current.type != EventType.Repaint || !rect.Contains(Event.current.mousePosition) && !DebugViewSettings.drawTooltipEdges 
                                                    || tip.textGetter == null && tip.text.NullOrEmpty() || SteamDeck.KeyboardShowing)
            return;
        if (DebugViewSettings.drawTooltipEdges)
            Widgets.DrawBox(rect);
        if (!TooltipHandler.activeTips.ContainsKey(tip.uniqueId))
        {
            ActiveTip activeTip = new ActiveTip(tip);
